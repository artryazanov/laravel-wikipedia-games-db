...
class ProcessGamePageJobDispatchSeriesTest extends TestCase
{
    use RefreshDatabase;

    public function test_dispatches_series_jobs_for_linked_series(): void
    {
        config()->set('game-scraper.throttle_milliseconds', 0);

        $title = 'Game With Series Links';
        $html = '<html></html>';
        ...
        Bus::assertDispatched(ProcessSeriesPageJob::class, fn ($job) => $job->pageTitle === 'The Legend of Zelda');
        Bus::assertDispatched(ProcessSeriesPageJob::class, fn ($job) => $job->pageTitle === 'Mario (franchise)');
        Bus::assertDispatchedTimes(ProcessSeriesPageJob::class, 2);
    }
}
